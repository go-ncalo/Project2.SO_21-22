...
static pthread_mutex_t open_files_mutex;
...
int tfs_init() {
    ...
    if (pthread_mutex_init(&init_mutex, 0) != 0)
        return -1;

    if (pthread_mutex_init(&open_files_mutex, 0) != 0)
        return -1;
...
int tfs_destroy() {
    ...
    if (pthread_mutex_destroy(&open_files_mutex) != 0) {
        return -1;
    }
